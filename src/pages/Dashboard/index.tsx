...
const Dashboard = () => {
  ...
  const paymentBreakdown = ["cash", "card", "transfer", "qris"].map(
    (method) => {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        <StatsCard
          title="Total Revenue"
          stat={formatIDR(totalRevenue)}
          icon={TrendingUp}
          variant="orange"
        />
        <StatsCard
          title="Completed Orders"
          stat={completedTransactions.length}
          icon={ReceiptText}
          variant="blue"
        />
        <StatsCard
          title="Avg Order Value"
          stat={formatIDR(avgOrderValue)}
          icon={ShoppingCart}
          variant="green"
        />
        <StatsCard
          title="Voided Orders"
          stat={voidedTransactions.length}
          icon={Ban}
          variant="red"
        />
      </div>
            ...
            <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-lg border border-gray-200">
              {(["daily", "weekly", "monthly"] as Period[]).map((p) => (
                <button
                  key={p}
                  onClick={() => setPeriod(p)}
                  className={`px-3 py-1 rounded-md text-xs font-medium transition-all capitalize ${period === p
                ...
                <div key={p.method}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs text-gray-500">
                      {paymentTotal > 0
                        ? Math.round((p.total / paymentTotal) * 100)
                        : 0}%
                    </span>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-2">
                    <div
                      ...
                      style={{
                        width: `${paymentTotal > 0 ? (p.total / paymentTotal) * 100 : 0}%`,
                      }}
                    />
                  </div>
      ...
      <div className="grid grid-cols-3 gap-4 items-stretch h-[320px]">
          ...
          {lowStockProducts.length === 0 && outOfStockProducts.length === 0 ? (
            ...
            <div className="flex flex-col gap-2 overflow-y-auto flex-1 min-h-0 pr-1">
              {outOfStockProducts.map((p) => (
                <div key={p.product_id} className="flex items-center justify-between p-2.5 bg-red-50 rounded-lg shrink-0">
              ...
              {lowStockProducts.map((p) => (
                <div key={p.product_id} className="flex items-center justify-between p-2.5 bg-yellow-50 rounded-lg shrink-0">
                  <div className="flex items-center gap-2 min-w-0 flex-1">
        ...
        {/* Recent Transactions */}
        <div className="col-span-2 bg-white rounded-lg shadow p-3 flex flex-col overflow-hidden">  {/* ← added flex flex-col overflow-hidden */}
          <div className="flex items-center justify-between mb-4 shrink-0">
          ...
          {recentTransactions.length === 0 ? (
            ...
            <div className="flex flex-col gap-1 overflow-y-auto flex-1 min-h-0">  {/* ← scrollable too */}
              {recentTransactions.map((t) => {
                const date = new Date(t.created_at);
                return (
                  <div key={t.transaction_id} className="flex items-center justify-between px-3 py-2.5 rounded-lg hover:bg-gray-50 transition-colors shrink-0">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {t.is_voided ? <Badge label="Voided" variant="red" /> : <Badge label="Completed" variant="green" />}
                    </div>
                  </div>
                );
              })}
